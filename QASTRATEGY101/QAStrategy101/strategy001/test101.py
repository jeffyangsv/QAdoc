...
def split_data(data, un=5)
    ...
    return pd.Series([data.iloc[step(n-1)stepn].index.tolist() for n in range(1,un+1) ], index=['l{}'.format(n) for n in range(1,un+1)])
def get_all(res, level, timerange=50)
    ...
    return pd.Series(zres.iloc[1timerange+1].index.map(y), index= zres.iloc[1timerange+1].index, name=level).cumsum()
def test_factor(x,layer=5, timerange=50)
    res = x.apply(lambda xsplit_data(x, layer), axis=1)
